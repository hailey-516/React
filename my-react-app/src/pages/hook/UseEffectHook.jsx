...
/*
    * useEffect
        : 컴포넌트가 렌더링될 때 특정 작업을 수행할 수 있도록 해 주는 리액트 훅
            클래스형 컴포넌트의 생명주기 메소드의 역할을 함

        - 컴포넌트가 마운트되었을 때 (처음 렌더링됐을 때)
        - 언마운트 되었을 때 (사라질 때)
...
export const UseEffectHook = () => {
    ...
    useEffect(()=>{
        // 렌더링되었을 때, state가 변경될 때마다 실행
        // componentDidMount, cocmponentDidUpdate의 역할
        console.log("의존성 배열이 없는 useEffect");
    });

    useEffect(()=>{
        // 렌더링되었을 때 한 번만 실행 (최초 마운트)
        // componentDidMount의 역할
        console.log("빈 배열을 의존성 배열로 가지는 useEffect");
    }, []);

    useEffect(()=>{
        // 렌더링되었을 때, name이라는 상태(state)가 변경되었을 때마다 실행
